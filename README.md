...
Its `source` is the actual JSON filename loaded from `--catalog`, even when that
filename differs from the connector ID. Library callers that pass a catalog
directly receive the stable fallback `<connector-id>.json` source.
